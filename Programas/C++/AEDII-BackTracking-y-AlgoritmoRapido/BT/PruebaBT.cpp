...
int main(){
	...
	uniform_int_distribution<> distrib2(1, 15);
    ...
    for (int i = 0; i < C; i++){	//Una iteracion por cada tipo de prenda para rellenar la tabla
        ...
        for (int j = 0; j < k; j++){
			prendas[i][j] = distrib(gen);	//Precio de cada modelo de cada prenda
        }
    }
        
    for (auto& fila : prendas) {
		sort(fila.begin(), fila.end(), greater<int>());	//Ordenacion de mayor a menor de cada fila de manera que es mas probable encontrar 													//la solucion antes suponiendo que los presupuestos suelen ser relativamente altos
	}
