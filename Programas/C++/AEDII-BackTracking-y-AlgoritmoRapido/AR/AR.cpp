...
int main(){
	...
	int P;
    cin >> P;	//Se lee el numero de casos de prueba
    
    cout << endl << P << endl;
    
    for (int caso = 0; caso < P; caso++) {	//Se resuelve el problema una vez para cada caso

        int M, A;
        cin >> M >> A;	//Se leen los mecanicos y averias de este caso
        
        vector<vector<int>> C(M, vector<int>(A, 0));	//Declaracion de la tabla 
        
        for (int i = 0; i < M; i++) {
            for (int j = 0; j < A; j++) {
                cin >> C[i][j];	//Se leen los valores de cada tabla de entrada
            }
        }
