...
string GeneradorCadenasSolucionAlInicio(int longitud, int m){	//Crea la cadena con la solucion al principio
	...
	resultado += GeneradorCadenas(longitud - m);

    return resultado;
}

string GeneradorCadenasSolucionAlFinal(int longitud, int m){	//Crea la cadena con la solucion al final
...
string GeneradorCadenasSolucionEnMedio(int longitud, int m){	//Crea la cadena con la solucion en el medio
	
	int generados = longitud - m;
    int izquierda   = generados / 2;
    int derecha = generados - izquierda;

    string parteIzquierda = GeneradorCadenas(izquierda);
    string parteDerecha  = GeneradorCadenas(derecha);
    ...
    return parteIzquierda + resultado + parteDerecha; 
}

array<string, tam*4> GeneradorCasos(int m){

	int casos[] = {600000, 700000, 800000, 900000, 1000000};	//Longitud de cadenas a probar 
	array<string, tam*4> cadenas; 
	
	for(int i = 0; i < tam; i++){
		cadenas[i] = GeneradorCadenas(casos[i]);		//Se generan las cadenas aleatorias
		cadenas[i+5] = GeneradorCadenasSolucionAlInicio(casos[i], m);	//Se generan las cadenas con solucion en el inicio
		cadenas[i+10] = GeneradorCadenasSolucionEnMedio(casos[i], m);	//Se generan las cadenas con solucion en el medio
		cadenas[i+15] = GeneradorCadenasSolucionAlFinal(casos[i], m);	//Se generan las cadenas con solucion en el final
	}
	return cadenas;
}
...
int main(){
	...
	int m = 5;
	int n = 0;
	struct timeval ti, tf;	//Para medir el tiempo con precision
	double tiempo;
	
	
	array<string, tam*4> cadenasGeneradas = GeneradorCasos(m);	//Se crea un array capaz de guardar 10 cadenas y lo rellenamos llamando al generador de casos
	...
	for(int i = 0; i < tam*4; i++) {
		...
		tiempo = ((tf.tv_sec - ti.tv_sec)*1000 + (tf.tv_usec - ti.tv_usec)/1000.0)/1000.0;	//Calculo de los segundos
		cout << "Posicion de inicio es " << solucion.second << " diferencia total maxima igual a " << solucion.first << endl;	//Se imprime el resultado
		cout << "Tamaño cadena: " << n << " " << "Tiempo en segundos: " << tiempo << endl;
		cout << endl;
   }	
