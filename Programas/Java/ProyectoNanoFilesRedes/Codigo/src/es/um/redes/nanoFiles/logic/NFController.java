...
public class NFController {
	...
	private static final byte FINISHED = 4;
	private static final byte WAIT_ACK_REQUESTFILELIST = 5;
	private static final byte RETRY_REQUESTFILELIST = 6;
	private static final byte WAIT_ACK_PUBLISHFILES = 7;
	private static final byte RETRY_PUBLISHFILES = 8;
	...
	public void processCommand() {
		...
		case NFCommands.COM_DOWNLOAD:
			...
			if (NanoFiles.testModeTCP) {
				...
				FileWithPeers fileInfo = controllerDir.getFilenameInfo(targetFilenameSubstring);
				if (fileInfo == null) {
	...
	private boolean canProcessCommandInCurrentState() {
		...
		case NFCommands.COM_QUIT:
			...
			break;
		case NFCommands.COM_PING:
			if (currentState != OFFLINE && currentState != RETRY_PING) {
				commandAllowed = false;
				System.err.println("* No puedes hacer ping si no estás offline o reintentando ping");
			}
			break;
		case NFCommands.COM_FILELIST:
			if (currentState != ONLINE && currentState != WAIT_ACK_REQUESTFILELIST
					&& currentState != RETRY_REQUESTFILELIST) {
				commandAllowed = false;
				System.err.println(
						"* No puedes solicitar la lista de archivos si no estás online o esperando confirmación");
			}
			break;
		case NFCommands.COM_SERVE:
			if (currentState != ONLINE) {
				commandAllowed = false;
				System.err.println("* No puedes servir archivos si no estás online");
			}
			break;
		case NFCommands.COM_DOWNLOAD:
			if (currentState != ONLINE) {
				commandAllowed = false;
				System.err.println("* No puedes descargar archivos si no estás online");
			}
			break;
		case NFCommands.COM_UPLOAD:
			if (currentState != ONLINE && currentState != WAIT_ACK_PUBLISHFILES && currentState != RETRY_PUBLISHFILES) {
				commandAllowed = false;
				System.err.println("* No puedes subir archivos si no estás online o esperando confirmación");
			}
			break;
		...
		case NFCommands.COM_SOCKET_IN:
			if (currentState == OFFLINE || currentState == FINISHED) {
				commandAllowed = false;
				System.err.println("* No puedes recibir conexiones si estás offline o finalizado");
			}
			break;
		default:
			System.err.println("ERROR: Comando no reconocido en este estado");
			commandAllowed = false;
		}
	...
	private void updateCurrentState(boolean success) {
		...
		case NFCommands.COM_PING:
			...
			break;
		case NFCommands.COM_FILELIST:
			currentState = WAIT_ACK_REQUESTFILELIST;
			break;
		case NFCommands.COM_UPLOAD:
			currentState = WAIT_ACK_PUBLISHFILES;
			break;
		case NFCommands.COM_SOCKET_IN:
			if (currentState == WAIT_ACK_PING) {
				currentState = ONLINE;
			} else if (currentState == WAIT_ACK_REQUESTFILELIST) {
				currentState = ONLINE;
			} else if (currentState == WAIT_ACK_PUBLISHFILES) {
				currentState = ONLINE;
			}
			break;
		case NFCommands.COM_QUIT:
