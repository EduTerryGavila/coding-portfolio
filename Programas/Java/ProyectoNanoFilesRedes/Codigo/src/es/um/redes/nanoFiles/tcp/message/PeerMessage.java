...
public class PeerMessage {
	...
	private byte opcode;
	public static final int VALOR_INVALIDO_FRAGMENTO = -1;
	/*
	...
	public PeerMessage(byte op, String reqFileHash) {
		opcode = op;
		this.requFileHash = hexStringToBytes(reqFileHash);
	}
	
	public PeerMessage(byte op, int sizeName, String fileName) {
		opcode = op;
		size = sizeName;
		requFileHash = fileName.getBytes();
	}
	...
	public String getReqFileHash() {
		if (opcode != PeerMessageOps.OPCODE_REQUEST_FILE && opcode != PeerMessageOps.OPCODE_UPLOAD && opcode != PeerMessageOps.OPCODE_FILENAME_TO_SAVE) {
			throw new StructureViolationException("This instance does not support getFileName. Check \'getOpcode() == PeerMessageOps.OPCODE_REQUEST_FILE\' first");
		}
		return bytesToHex(requFileHash);
	}
	...
	public String getFileName() {
		...
		return new String(requFileHash);
	}
	
	private void setReqFileHash(String reqFileHash) {
		if (opcode != PeerMessageOps.OPCODE_REQUEST_FILE) 
	...
	//estas funciones son para convertir una cadena a un array de byte y viceversa, las usamos para los hashs de los ficheros entre otros para poder imprimirlos
	...
	public static PeerMessage readMessageFromInputStream(DataInputStream dis) throws IOException {
		...
		System.out.println("opcode leido en readMessageFromInputStream: " + opcode);
		switch (opcode) {
		...
		case PeerMessageOps.OPCODE_REQUEST_FILE_ACCEPTED:
		case PeerMessageOps.OPCODE_FILE_NOT_FOUND:
		case PeerMessageOps.OPCODE_REQUEST_CHUNK_OUTOFRANGE:
			break;
	default:
